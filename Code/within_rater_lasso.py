...
def eval_one_rater(df_r):
    ...
    model = Pipeline([
        ...
        ("est", Lasso(alpha=0.001, random_state=42, max_iter=20000))
    ])
    model.fit(X_tr, y_tr)
...
def main():
    ...
    out_csv = CSV_DIR / f"all_raters_{TARGET}_lasso_scores.csv"
    ...
    out_top2 = CSV_DIR / f"top2_{TARGET}_lasso.csv"
    ...
    ax.set_title(f"Within-Rater LASSO — Target: {TARGET}")
    ax.set_xlabel("Rater"); ax.set_ylabel("Correlation")
    ax.set_ylim(0, 1)
    ax.legend(); fig.tight_layout()
    png_path = PLOT_DIR / f"all_raters_{TARGET}_lasso_bar.png"
