...
for df in (raw, train_items, test_items):
    if id_col not in df.columns:
        raise RuntimeError(f"ID column '{id_col}' not found in one of the split files. Check your split CSVs.")
...
if len(train_df) == 0 or len(test_df) == 0:
    raise RuntimeError(f"After merge, empty train/test. Train n={len(train_df)}, Test n={len(test_df)}. "
                       "Check that split IDs match Painting IDs in the data file.")
...
print(f"[RIDGE] Target={TARGET} | alpha*={model.alpha_:.5f}")
...
csv_path = CSV_DIR / f"RIDGE_metrics_{TARGET}.csv"
...
plt.title(f"True vs Predicted — RIDGE ({TARGET})")
plt.grid(True, linestyle="--", alpha=0.6)
plt.tight_layout()
plot_path = PLOT_DIR / f"RIDGE_true_vs_pred_{TARGET}.png"
