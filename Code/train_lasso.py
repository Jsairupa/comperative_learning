...
for df in (raw, train_items, test_items):
    if id_col not in df.columns:
        raise RuntimeError(f"ID column '{id_col}' not found in one of the split files. Check your train/test split CSVs.")
    df[id_col] = df[id_col].astype(str).str.strip()


needed = [id_col, TARGET] + FEATURES
...
if len(train_df) == 0 or len(test_df) == 0:
    raise RuntimeError(f"After merge, empty train/test. Train n={len(train_df)}, Test n={len(test_df)}. "
                       "Check that Painting IDs in split files match the data file.")
...
model = LassoCV(alphas=np.logspace(-3, 1, 15), max_iter=30000, random_state=42)
...
print(f"[LASSO] Target={TARGET} | alpha*={model.alpha_:.5f}")
...
csv_path = CSV_DIR / f"LASSO_metrics_{TARGET}.csv"
...
plt.title(f"True vs Predicted — LASSO ({TARGET})")
plt.grid(True, linestyle="--", alpha=0.6)
plt.tight_layout()
plot_path = PLOT_DIR / f"LASSO_true_vs_pred_{TARGET}.png"
