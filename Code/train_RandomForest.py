...
# detect id column and cast to str
...
if len(train_df) == 0 or len(test_df) == 0:
    raise RuntimeError(f"After merge, empty train/test. Train n={len(train_df)}, Test n={len(test_df)}. "
                       "Check that split IDs match the Painting IDs in the data file.")
...
csv_path = CSV_DIR / f"RF_metrics_{TARGET}.csv"
...
plt.title(f"True vs Predicted — RF ({TARGET})")
plt.grid(True, linestyle="--", alpha=0.6)
plt.tight_layout()
plot_path = PLOT_DIR / f"RF_true_vs_pred_{TARGET}.png"
